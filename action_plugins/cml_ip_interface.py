...
class ActionModule(ActionBase):
    ...
    CONFIG_RE = re.compile(r'net add interface (?P<interface>(?!eth0)\S+) (?P<config>.*)')
        ...
        if len(running) > 0:
            for interface, configs in _running['data'].items():
                if interface not in _intent['data']:
                    _del.append('del interface %s' % interface)
                    continue

                for config in configs:
                    if config not in _intent['data'][interface]:
                        _del.append('del interface %s %s' % (interface, config))
