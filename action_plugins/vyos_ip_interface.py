...
class ActionModule(ActionBase):
    ...
    CONFIG_RE = re.compile(
        r'set (?P<interface>interfaces ethernet \w+\D(\d+)( vif \d+|)) (?P<config>(?=address|description).*)')
    ...
    def _get_commands(self, running, intent, dev_os):
        ''' Return a list of net set/delete commands based on running and intent config '''
        ...
        if len(running) > 0:
            for interface, configs in _running['data'].items():
                if interface not in _intent['data']:
                    _del.append('delete %s' % interface)
                    continue

                for config in configs:
                    if config not in _intent['data'][interface]:
                        _del.append('delete %s %s' % (interface, config))
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        commit = not task_vars['ansible_check_mode']
